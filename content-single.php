...
<article id="post-<?php the_ID(); ?>" <?php post_class( $postclass ); ?>>
  ...
  <header class="entry-header">
    <?php if ( has_post_thumbnail() || get_header_image() ) : ?>
      <?php if ( has_post_thumbnail() ) : ?>
        <div class="entry-thumbnail blury-header" style="background-image: url(<?php echo esc_url( $thumbnail[0] ); ?>);"></div>
      <?php else : ?>
        <div class="header-image blury-header" style="background-image: url(<?php header_image(); ?>)"></div>
      <?php endif; ?>
    <?php endif; ?>
  ...
  <div class="entry-meta">
    ...
    // creating a short url with jnth.co
    $shortUrl = file_get_contents("http://jnth.co/shorten/".$url);

    if($shortUrl){
      $evenShorter = str_replace("http://", "", $shortUrl);
      //echo $evenShorter;
      echo "</span><span class='byline'><span class='sep'> &bull; </span>$evenShorter</span>" ;
    }
    
    
    
    
    echo '<br><div class="fb-share-button" data-href="'.$url.'" data-layout="button"></div><br>';
...
?>
  ...
  <div class="entry-content">
    ...
    <?php the_content(); 
    $intro = '<div class="entry-meta">';
    $outro = " </div>";
    echo $intro."Partagez l'article sur Facebook<br>".'<div class="fb-share-button" data-href="'.$url.'" data-layout="button"></div>'.$outro;
